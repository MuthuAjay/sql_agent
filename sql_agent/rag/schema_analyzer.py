...
from ..utils.schema_utils import (
    prepare_analysis_context, extract_business_keywords, 
    detect_naming_patterns, detect_potential_relationships,
    calculate_basic_quality_score, smart_sample_data,
    prepare_table_metadata, generate_cache_key
)
    ...
    def _initialize_llm(self) -> None:
        ...
        try:
            if settings.enable_business_intelligence:
                ...
                self.logger.info("llm_provider_initialized", 
...
- User personas who interact with this data
- Business processes this supports
- Criticality to business operations
- Optimization opportunities
- Regulatory/compliance considerations

AVOID: Technical jargon. Focus on business meaning and value.""",
...
{{
  ...
  "workflow_optimization": ["opportunity1", "opportunity2"]
    ...
    async def analyze_database_intelligence(
        ...
        """
        try:
            self.logger.info("database_intelligence_analysis_start",
            ...
            self.logger.info("database_intelligence_analysis_complete",
                           database=schema_data.get("database_name"),
            ...
            self.logger.error("database_intelligence_analysis_failed",
                            database=schema_data.get("database_name"),
                            error=str(e), exc_info=True)
            return self._create_error_analysis(schema_data, str(e))
    
    async def _discover_database_purpose(self, schema_data: Dict[str, Any]) -> DatabaseIntelligence:
    ...
    async def _process_table_batch(
        ...
        for table in tables:
            try:
                ...
                context = await self._analyze_single_table_context(table, analysis_level)
                batch_results[table_name] = context
            except Exception as e:
                self.logger.warning("table_context_analysis_failed",
                                  table=table.get("name"), error=str(e))
                batch_results[table.get("name", "")] = self._create_minimal_table_context(table.get("name", ""))
    ...
    async def _analyze_single_table_context(
        ...
        try:
            ...
            self.logger.error("analyze_single_table_context_failed",
                            table=table_data.get("name"), error=str(e))
    ...
    def _analyze_database_naming_patterns(self, tables: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _analyze_relationship_patterns(self, tables: List[Dict[str, Any]]) -> str:
        ...
        density = "high" if total_relationships > len(tables) * 1.5 else "medium" if total_relationships > len(tables) * 0.5 else "low"
    ...
    async def _enhance_relationships_with_context(
        ...
        for rel in relationships:
            ...
            if source_table in table_contexts and target_table in table_contexts:
                ...
                common_processes = set(source_context.business_processes) & set(target_context.business_processes)
    ...
    async def _execute_llm_analysis(
        ...
        try:
            ...
            self.logger.error("execute_llm_analysis_failed",
    ...
    def _parse_llm_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": f"JSON parse failed: {str(e)}", "raw_response": response[:500]}
        except Exception as e:
    ...
    def _parse_database_intelligence(self, analysis_result: Dict[str, Any]) -> DatabaseIntelligence:
        ...
        try:
            ...
            return DatabaseIntelligence(
                business_purpose=analysis_result.get("business_purpose", "Database system"),
                ...
                optimization_priorities=analysis_result.get("optimization_priorities", [])
            )
    ...
    def _parse_table_business_context(self, analysis_result: Dict[str, Any]) -> TableBusinessContext:
        ...
        try:
            return TableBusinessContext(
                ...
                data_characteristics=analysis_result.get("data_characteristics", "Mixed data"),
    ...
    def _calculate_confidence_metrics(
        ...
        if len(database_intelligence.discovered_domains) > 0:
            avg_domain_confidence = sum(d.confidence for d in database_intelligence.discovered_domains) / len(database_intelligence.discovered_domains)
        ...
        if table_contexts:
            ...
            metrics["avg_table_confidence"] = sum(table_confidences) / len(table_confidences)
        else:
    ...
    def _create_error_analysis(self, schema_data: Dict[str, Any], error: str) -> AnalysisResult:
        """Create error analysis result."""
        return AnalysisResult(
            database_intelligence=self._create_minimal_database_intelligence(schema_data),
            table_contexts={},
            semantic_relationships=[],
            business_workflow_map={},
    ...
    def _create_minimal_database_intelligence(self, schema_data: Dict[str, Any]) -> DatabaseIntelligence:
    ...
    async def quick_table_analysis(self, table_data: Dict[str, Any]) -> TableBusinessContext:
    ...
    async def discover_business_domains(self, schema_data: Dict[str, Any]) -> List[BusinessDomain]:
        """Discover business domains in the database."""
        analysis_result = await self.analyze_database_intelligence(schema_data, AnalysisLevel.STANDARD)
    ...
    def get_analysis_summary(self, analysis_result: AnalysisResult) -> Dict[str, Any]:
        ...
        return {
            ...
            "discovered_domains_count": len(analysis_result.database_intelligence.discovered_domains),
            ...
            "overall_confidence": analysis_result.confidence_metrics.get("overall_confidence", 0),
            ...
            "utilities_used": analysis_result.analysis_metadata.get("utilities_used", False)
        }
    
    def export_for_vector_storage(self, analysis_result: AnalysisResult) -> Dict[str, Any]:
    ...
    async def enhance_existing_schema(
        ...
        try:
            ...
            enhanced_schema["business_intelligence"] = {
                "business_purpose": analysis_result.database_intelligence.business_purpose,
                "industry_domain": analysis_result.database_intelligence.industry_domain,
                "discovered_domains": [asdict(domain) for domain in analysis_result.database_intelligence.discovered_domains],
                "confidence_metrics": analysis_result.confidence_metrics,
            ...
            for table in enhanced_schema.get("tables", []):
                ...
                if table_name in analysis_result.table_contexts:
                    ...
                    table["business_keywords_str"] = ",".join(filter(None, business_keywords))
            ...
            self.logger.info("schema_enhancement_complete", 
                           table_count=len(enhanced_tables),
                           domains_discovered=len(analysis_result.database_intelligence.discovered_domains))
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        return {
            **self.config,
            ...
            "llm_provider": settings.effective_llm_provider if self._llm_provider else None,
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self._llm_provider:
            try:
                test_messages = [
                    ...
                    {"role": "user", "content": "Respond with 'OK' to confirm connectivity."}
                ]
                response = await self._llm_provider.generate(test_messages)
                health_status["llm_test"] = "passed"
                health_status["llm_response_preview"] = response[:50] if response else "empty"
            except Exception as e:
        ...
        try:
            test_keywords = extract_business_keywords("test_table", ["id", "name", "email"])
