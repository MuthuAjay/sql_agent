...
class VectorStore:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.client = chromadb.PersistentClient(
                path=settings.chroma_db_path or "./chroma_db", settings=chroma_settings
            )
            ...
            self.logger.error(
                "enhanced_vector_store_initialization_failed",
                error=str(e),
                exc_info=True,
            )
            raise
    ...
    def _ensure_initialized(self) -> None:
        ...
        if not self._initialized:
            raise RuntimeError(
                "Enhanced vector store not initialized. Call initialize() first."
            )
    ...
    def _create_table_contexts(
        self, table_data: Dict[str, Any], database_name: str
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(foreign_keys) > 0:
                relationship_context = self._create_relationship_context(
            ...
            self.logger.error(
                "create_table_contexts_failed", table=table_name, error=str(e)
            )
            return []

    def _create_table_level_context(
        self, table_data: Dict[str, Any], database_name: str
    ) -> Dict[str, Any]:
        ...
        description = table_data.get("enhanced_description") or table_data.get(
            "description", ""
        )
        if description:
        ...
        if columns:
            key_columns = [
                ...
                for col in columns
                if any(
                    keyword in col.lower() for keyword in ["id", "key", "name", "email"]
                )
            ]
            if key_columns:
        ...
        if sample_data.get("sample_count", 0) > 0:
            content_parts.append(
                f"Contains {sample_data['sample_count']} example records"
            )
        ...
        metadata = {
            ...
            "business_concepts": table_data.get(
                "business_concepts_str",
                ",".join(business_concepts) if business_concepts else "",
            ),
            "semantic_tags": table_data.get(
                "semantic_tags_str", ",".join(semantic_tags) if semantic_tags else ""
            ),
            "has_relationships": len(table_data.get("foreign_keys", [])) > 0,
            "data_quality_score": table_data.get("data_quality", {}).get(
    ...
    def _create_column_level_context(
        ...
        metadata = {
            ...
            "is_primary_key": column_name
            in [pk.get("column", "") for pk in table_data.get("primary_keys", [])],
            "is_foreign_key": column_name
            in [fk.get("column", "") for fk in table_data.get("foreign_keys", [])],
            "created_at": datetime.utcnow().isoformat(),
            "search_keywords": ",".join(
                [column_name, data_type, business_concept]
                if business_concept
    ...
    def _create_relationship_context(
        self, table_data: Dict[str, Any], database_name: str
    ) -> Dict[str, Any]:
        ...
        for fk in foreign_keys:
            ...
            if ref_table:
                ...
                content_parts.append(
                    f"Links to {ref_table} via {fk_column} -> {ref_column}"
                )
        ...
        metadata = {
            ...
            "relationship_strength": table_data.get("relationship_insights", {}).get(
    ...
    def _extract_search_keywords(self, table_data: Dict[str, Any]) -> List[str]:
        ...
        key_columns = [
            ...
            if any(
                ...
                for keyword in ["name", "email", "type", "status"]
            )
        ]
    ...
    async def _add_contexts_batch(self, contexts: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            self.logger.error(
                "add_contexts_batch_failed", error=str(e), count=len(contexts)
            )
            raise

    async def search_for_table_selection(
        self, search_context: SchemaSearchContext
        ...
        try:
            ...
            if cached_results:
                self.logger.debug(
                    "table_selection_cache_hit", query=search_context.query[:50]
                )
    ...
    async def _search_by_text_enhanced(
        self, search_context: SchemaSearchContext
    ) -> List[VectorSearchResult]:
    ...
    async def _search_by_business_concepts(
        self, search_context: SchemaSearchContext
    ) -> List[VectorSearchResult]:
        ...
        try:
            ...
            for concept in concepts:
                ...
                try:
                    chroma_results = self.collection.query(
                        query_texts=[concept], n_results=5, where=filters
                    )

                    results = self._convert_chroma_results(
                        chroma_results, "business_concept"
                    )
                    concept_results.extend(results)

                except Exception as e:
                    self.logger.debug(
                        "business_concept_search_failed", concept=concept, error=str(e)
                    )
                    continue
    ...
    async def _search_by_keywords(
        ...
        try:
            ...
            for keyword in keywords[:3]:  # Limit to top 3 keywords
                ...
                try:
                    chroma_results = self.collection.query(
                        query_texts=[keyword], n_results=3, where=filters
                    )

                    results = self._convert_chroma_results(
                        chroma_results, "keyword_match"
                    )
                    keyword_results.extend(results)

                except Exception as e:
                    self.logger.debug(
                        "keyword_search_failed", keyword=keyword, error=str(e)
                    )
                    continue
    ...
    def _convert_chroma_results(
        self, chroma_results: Dict, search_type: str
    ) -> List[VectorSearchResult]:
        ...
        for i in range(len(chroma_results["ids"][0])):
            ...
            result = VectorSearchResult(
                ...
                embedding=(
                    chroma_results["embeddings"][0][i]
                    if chroma_results.get("embeddings")
                    else None
                ),
            )
    ...
    def _rank_and_filter_results(
        self, results: List[VectorSearchResult], search_context: SchemaSearchContext
    ) -> List[VectorSearchResult]:
        ...
        for result in results:
            ...
            if result.table_name.lower() in query_lower:
                boosted_score += self.config["relevance_boost_factors"][
                    "exact_table_match"
                ]
            ...
            for concept in result.business_concepts:
                if concept.lower() in query_lower:
                    boosted_score += self.config["relevance_boost_factors"][
                        "business_concept_match"
                    ]

            # Boost for data type relevance
            if result.data_type and any(
                word in query_lower for word in ["number", "text", "date", "time"]
            ):
                boosted_score += self.config["relevance_boost_factors"][
                    "data_type_match"
                ]
    ...
    async def _clear_database_schema(self, database_name: str) -> None:
        ...
        try:
            ...
            if results["ids"]:
                ...
                self.logger.info(
                    "database_schema_cleared", database=database_name, count=len(ids)
                )

        except Exception as e:
            self.logger.warning(
                "clear_database_schema_failed", database=database_name, error=str(e)
            )

    # Public API methods

    async def get_tables_for_query(
        self, query: str, database_name: str, limit: int = 5
    ) -> List[str]:
        ...
        for result in results:
            ...
            if table_name:
                ...
                table_scores[table_name] = max(
                    current_score, result.relevance_score * weight
                )
    ...
    async def similarity_search(
        ...
        This method is expected by the context manager for vector-based retrieval.
        """
        self._ensure_initialized()
        ...
        try:
            ...
            if filters:
                # Convert to ChromaDB format with proper syntax
                if len(filters) == 1:
            ...
            results = self._convert_chroma_results(chroma_results, "similarity_search")

            # Filter by distance thresholds
            filtered_results = [
                result
                for result in results
                if min_distance <= result.distance <= max_distance
            ]
    ...
    async def get_by_table_name(self, table_name: str, database_name: str = "default") -> List[VectorSearchResult]:
        """
        Get all contexts for a specific table.
        
        This method is expected by the context manager for table-specific retrieval.
        """
        self._ensure_initialized()
        
        try:
            self.logger.debug("get_by_table_name_started", 
                            table=table_name, 
                            database=database_name)
            
            # Use proper ChromaDB syntax for multiple filters
            filters = {
                "$and": [
                    {"database_name": database_name},
                    {"table_name": table_name}
            ...
            # Get all contexts for this table
            ...
            results = []
            if chroma_results.get('ids') and chroma_results['ids']:
                for i in range(len(chroma_results['ids'])):
                    ...
                    result = VectorSearchResult(
                        ...
                        relevance_score=1.0,  # Perfect relevance for exact table match
                        table_name=metadata.get("table_name", ""),
                        column_name=metadata.get("column_name"),
                        data_type=metadata.get("data_type"),
                        business_concepts=metadata.get("business_concepts", "").split(",") if metadata.get("business_concepts") else [],
                        embedding=chroma_results['embeddings'][i] if chroma_results.get('embeddings') else None
                    )
                    results.append(result)
            
            self.logger.info("get_by_table_name_completed",
                            table=table_name,
                            database=database_name,
            ...
            self.logger.error("get_by_table_name_failed",
                            table=table_name,
                            database=database_name, 
                            error=str(e),
                            exc_info=True)
    ...
    async def search_by_text(self, text: str, limit: int = 10, database_name: str = "default") -> List[VectorSearchResult]:
        """
        Search by text using ChromaDB's built-in text search.
        
        This method is expected by the context manager for text-based fallback.
        """
        self._ensure_initialized()
        
        try:
            self.logger.debug("search_by_text_started", 
                            text=text[:50], 
                            limit=limit,
                            database=database_name)
            
            # Use ChromaDB text search
            chroma_results = self.collection.query(
                query_texts=[text],
                n_results=limit,
            ...
            # Convert to VectorSearchResult objects
            results = self._convert_chroma_results(chroma_results, "text_search")
            
            self.logger.info("search_by_text_completed",
                            text=text[:50],
                            database=database_name,
                            result_count=len(results))
            ...
            self.logger.error("search_by_text_failed",
                            text=text[:50],
                            database=database_name,
                            error=str(e),
                            exc_info=True)
    ...
    async def clear_collection(self) -> None:
        """
        Clear all contexts from the collection.
        
        This method is expected by the context manager for cache refresh.
        """
        self._ensure_initialized()
        
        try:
            self.logger.info("clearing_collection", collection=self.collection_name)
            
            # Clear all documents in the collection
            # ChromaDB doesn't have a clear_all method, so we delete all IDs
            all_results = self.collection.get()
            
            if all_results.get('ids') and all_results['ids']:
                ...
                self.logger.info("collection_already_empty", collection=self.collection_name)
            
        except Exception as e:
            self.logger.error("clear_collection_failed", 
                            collection=self.collection_name,
                            error=str(e),
                            exc_info=True)
            raise

    async def delete_by_table_name(self, table_name: str, database_name: str = "default") -> int:
        """
        Delete all contexts for a specific table.
        
        This method is expected by the context manager for table updates.
        """
        self._ensure_initialized()
        
        try:
            self.logger.info("delete_by_table_name_started",
                            table=table_name,
                            database=database_name)
            ...
            # Get all IDs for this table first
            table_results = self.collection.get(
                where=filters,
                include=["embeddings"]  # Only need IDs, but include something
            )
            
            deleted_count = 0
            if table_results.get('ids') and table_results['ids']:
                # Delete all contexts for this table
                self.collection.delete(ids=table_results['ids'])
                deleted_count = len(table_results['ids'])
            
            self.logger.info("delete_by_table_name_completed",
                            table=table_name,
                            database=database_name,
            ...
            self.logger.error("delete_by_table_name_failed",
                            table=table_name,
                            database=database_name,
                            error=str(e),
                            exc_info=True)
    ...
    async def get_table_context(
        ...
        try:
            ...
            results = self.collection.get(where={"$and": filters})
            ...
            self.logger.error(
                "get_table_context_failed", table=table_name, error=str(e)
            )
            return {}

    async def search_columns_for_query(
        ...
        try:
            ...
            filtered_results = [
                result for result in results if result.table_name in table_names
            ]
    ...
    async def get_relationship_insights(
        self, table_names: List[str], database_name: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(len(results["ids"])):
                ...
                if table_name in table_names:
                    ...
                    relevant_refs = [
                        ref for ref in referenced_tables if ref in table_names
                    ]
                    ...
                    if relevant_refs:
                        relationship_info = {
                            ...
                            "relationship_strength": metadata.get(
                                "relationship_strength", "unknown"
                            ),
                            "content": content,
                        }
    ...
    async def get_collection_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                "collection_stats_generated",
                **{k: v for k, v in stats.items() if k != "config"},
            )
            return stats

        except Exception as e:
            self.logger.error(
                "get_collection_stats_failed", error=str(e), exc_info=True
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            if self._initialized:
                ...
                test_results = await self.get_tables_for_query(
                    "test query", "test_db", limit=1
                )
    ...
    async def export_collection_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                "collection_data_exported", total_contexts=export_data["total_contexts"]
            )
            return export_data
